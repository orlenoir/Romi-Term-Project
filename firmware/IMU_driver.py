...
class BNO055:
    # ---- I2C addresses (depends on ADR pin on breakout; 0x28 is most common) ----
    ADDR_LOW = 0x28
    ADDR_HIGH = 0x29
    ...
    MODE_MAGGYRO = 0x06
    MODE_AMG = 0x07
    MODE_IMUPLUS = 0x08     # fusion (gyro+accel), no magnetometer
    MODE_COMPASS = 0x09
    MODE_M4G = 0x0A
    MODE_NDOF_FMC_OFF = 0x0B
    MODE_NDOF = 0x0C        # full fusion (gyro+accel+mag)
    ...
    def __init__(self, i2c, addr=ADDR_LOW, rst_pin=Pin.cpu.B5, do_reset=True):
        ...
        if chip != self.CHIP_ID:
            # Some boards need a moment after reset/power-up
            sleep_ms(50)
            chip = self._r8(self.REG_CHIP_ID)
            if chip != self.CHIP_ID:
    ...
    def hardware_reset(self):
        ...
        sleep_ms(10)
        self.rst.high()
        # Datasheet typical boot time; give it time to come up
        sleep_ms(700)

    def set_mode(self, mode):

        # Always go through CONFIG mode safely
    ...
    def read_cal_coeffs(self):
        ...
        # Offsets must be read in CONFIG mode on many setups
